...
const App = () => {
  ...
  return (
    <>
      <h1>Phonebook</h1>
      <div>Filter shown with <input type="text" value={newFilter} onChange={handleFilterChange} /></div>
      ...
      <h2>Numbers</h2>
      <ul>
        {personsFiltered.map(({ name, number }) => <li key={name + number}>{name} | {number}</li>)}
      </ul>
    </>
  )
}
